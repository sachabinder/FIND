...
def turntable(
    ...
    with tqdm(np.arange(nviews), disable=silent) as tqdm_it:
        ...
        for i in tqdm_it:
            ...
            transf = Transform3d(device=euler.device).rotate(
                euler_angles_to_matrix(euler, "XYZ")
            )
...
def render_dataset(gpu=0):
    ...
    dataset = Foot3DDataset(
        left_only=True, tpose_only=False, is_train=True, N=20, device=device
    )
    ...
    for i in gt_loader:
        turntable(
            i["mesh"],
            out_loc=os.path.join(out_dir, i["name"][0] + ".mp4"),
            nframes=250,
            gpu=gpu,
        )
