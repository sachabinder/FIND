...
def load_obj(loc, skip_materials=True):
    with open(loc, "rb") as f:
        mesh = trimesh.Trimesh(
            **trimesh.exchange.obj.load_obj(f, skip_materials=skip_materials)
        )
    return mesh
...
class NoTextureLoading:
    ...
    def __enter__(self, *args):
        for dataset in self.datasets:
            ...
            self.states.append(
                cur_state
            )  #  Store original states to restore back to when leaving context
...
class Foot3DDataset(Dataset):
    def __init__(
        ...
        self.data = [
            d
            for d in self.data
            if d["Foot ID"] not in cfg["TEMPLATE_FEET"] or specific_feet is not None
        ]
        ...
        if not (train_and_val or specific_feet):
            if is_train:
                self.data = [
                    d for d in self.data if d["Foot ID"] not in cfg["VAL_FEET"]
                ]
            else:
    ...
    def get_keys(self, idx):
        ...
        texkey = (
            shapekey
        ) = foot_id  #  These features should be shared across the same foot
    ...
    def get_by_id(self, ID):
        idx = [
            n
            for n, ann in enumerate(self.data)
            if ID == f"{ann['Foot ID']}-{ann['Scan ID']}"
        ]
    ...
    def __getitem__(self, idx):
        ...
        load_from_cache = (
            self.full_caching and name in _cache
        )  #  If caching mode, and this object has been cached, then load it
        ...
        if load_from_cache:
            ...
            if self._load_texture and cached_mesh.tex_img is None:
                load_from_cache = (
                    False  #  Do not load from cache as need to reload to get texture
                )

        if not load_from_cache:
            verts, face_dict, props = p3d_load_obj(
                obj_loc, device=self.device, load_textures=False
            )
            ...
            if self._load_texture:
                ...
                tex_img = (
                    torch.from_numpy(cv2.cvtColor(tex_img, cv2.COLOR_BGR2RGB)) / 255
                ).float()
        ...
        if self._load_texture:
            if cached_mesh.tex_img is not None:
                ...
                face_uvs = cached_mesh.face_dict.textures_idx.unsqueeze(0).to(
                    self.device
                )
                verts_uvs = cached_mesh.props.verts_uvs.unsqueeze(0).to(self.device)
                tex_map = TexturesUV(img, faces_uvs=face_uvs, verts_uvs=verts_uvs)
            else:
        ...
        out = {
            ...
            "kp_idxs": keypoints,
            "is_tpose": is_tpose,
            "orig_footedness": ann["footedness"],
            "pose_descr": ",".join(ann["pose"]),
            "pose_code": get_pose_code(ann["pose"]),
            **self.get_keys(idx),
        }
