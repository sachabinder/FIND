...
def vis_meshes(meshes):
    ...
    for n, mesh in enumerate(meshes):
        tri = trimesh.Trimesh(
            vertices=mesh.verts_packed().detach().numpy(),
            faces=mesh.faces_packed().detach().numpy(),
            process=False,
        )
...
def main(
    ...
    for i in range(gt_pts.shape[0]):
        ...
        chamf_cutoffs.append(
            chamfer_distance(
                gt_pts_z_cutoff.unsqueeze(0), pred_pts_z_cutoff.unsqueeze(0)
            )[0]
        )
    ...
    if produce_spins:
        ...
        for n in range(len(predictions)):
            ...
            x, x_lengths, _ = _handle_pointcloud_input(
                pred_mesh.verts_padded(), None, None
            )
            y, y_lengths, _ = _handle_pointcloud_input(
                gt_mesh.verts_padded(), None, None
            )

            x_sampled, x_sampled_lengths, _ = _handle_pointcloud_input(
                pred_pts[[n]], None, None
            )

            x_nn = knn_points(x, y, lengths1=x_lengths, lengths2=y_lengths, K=1)
            y_nn = knn_points(
                y, x_sampled, lengths1=y_lengths, lengths2=x_sampled_lengths, K=1
            )  # Has to be to x-sampled to account for low vertex count models (eg SUPR)
            ...
            renders = [
                Render(pred_mesh, pred_mesh.textures.clone(), "pred_rgb"),
                Render(pred_mesh, pred_chamf_tex, "pred_chamf"),
                Render(pred_mesh, pred_grey_tex, "pred_grey"),
                Render(gt_mesh, gt_mesh.textures.clone(), "gt_rgb"),
                Render(gt_mesh, gt_chamf_tex, "gt_chamf"),
                Render(gt_mesh, gt_grey_tex, "gt_grey"),
            ]
            ...
            for render in renders:
                ...
                turntable(
                    mesh,
                    out_loc=os.path.join(spins_dir, fname),
                    nframes=250,
                    silent=True,
                    azim=70,
                    dist=0.35,
    ...
    out_data[f"Chamf z-cutoff {z_cutoff} (μm)"] = (
        chamf_z_cutoff.cpu().detach().numpy() * 1e6
    )
    ...
    for i in range(len(foot_names)):
        for j in range(len(kp_labels)):
            text = ax.text(
                j, i, f"{dists[i, j]*1000:.1f}", ha="center", va="center", color="w"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no_rendering", action="store_true", help="Do not render feet"
    )
