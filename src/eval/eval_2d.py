...
def main(src, opts, exp_name=None, out_dir="eval_export/psnr", gpu=0):
    ...
    R, T = renderer.linspace_views(
        nviews=nviews, dist=0.3, elev_min=-90, elev_max=90
    )  # Sample linear arc for all models
    ...
    with tqdm(gt_loader) as tqdm_it:
        ...
        for batch in tqdm_it:
            ...
            for b in range(nviews // batch_size):
                subbatch_R, subbatch_T = (
                    R[b * batch_size : b * batch_size + batch_size],
                    T[b * batch_size : b * batch_size + batch_size],
                )
                gt_rdrs = renderer(
                    batch["mesh"],
                    subbatch_R,
                ...
                pred_rdrs = renderer(
                    res["meshes"], subbatch_R, subbatch_T, return_mask=True
                )
                ...
                *_, H, W, v = gt_rdrs["image"].shape
                ...
                out_data["MSE"].append(
                    MSE(gt_rdrs["image"], pred_rdrs["image"]).cpu().detach().numpy()
                )

                # Three methods of calculating PSNR, differing levels of masking
                out_data["PSNR_A"].append(
                    PSNR(gt_rdrs["image"], pred_rdrs["image"]).cpu().detach().numpy()
                )
                out_data["PSNR_B"].append(
                    PSNR(
                        gt_rdrs["image"] * gt_mask.unsqueeze(-1),
                        pred_rdrs["image"] * pred_mask.unsqueeze(-1),
                    )
                    .cpu()
                    .detach()
                    .numpy()
                )
                out_data["PSNR_C"].append(
                    PSNR(
                        gt_rdrs["image"] * common_mask.unsqueeze(-1),
                        pred_rdrs["image"] * common_mask.unsqueeze(-1),
                    )
                    .cpu()
                    .detach()
                    .numpy()
                ...
                out_data["IOU"].append(
                    IOU(gt_rdrs["mask"], pred_rdrs["mask"]).cpu().detach().numpy()
                )
    ...
    for batch in gt_loader:
        ...
        gt_kp = batch["mesh"].verts_packed()[batch["kp_idxs"].long()][
            0, [kp_labels.index("Big toe")]
        ]
        pred_kp = res["verts"][0][template_kp_idxs][[kp_labels.index("Big toe")]]

        Rpred, Tpred = look_at_view_transform(
            dist=0.15, elev=0, azim=0, up=((1, 0, 0),), at=pred_kp
        )
        Rgt, Tgt = look_at_view_transform(
            dist=0.15, elev=0, azim=0, up=((1, 0, 0),), at=gt_kp
        )
        ...
        *_, H, W, v = gt_rdrs["image"].shape
        ...
        # Apply GT masks to predicted
        pred_rdrs["image"][
            gt_rdrs["mask_out_masks"].unsqueeze(-1).expand_as(pred_rdrs["image"])
        ] = 1.0

        gt_stacked = np.hstack(
            gt_rdrs["image"].reshape(-1, H, W, 3).cpu().detach().numpy()
        )
        pred_stacked = np.hstack(
            pred_rdrs["image"].reshape(-1, H, W, 3).cpu().detach().numpy()
        )
    ...
    cv2.imwrite(
        os.path.join(out_dir, f"pred_HD.png"),
        cv2.cvtColor((np.vstack(out_pred) * 255).astype(np.uint8), cv2.COLOR_BGR2RGB),
    )
    cv2.imwrite(
        os.path.join(out_dir, f"gt_HD.png"),
        cv2.cvtColor((np.vstack(out_gt) * 255).astype(np.uint8), cv2.COLOR_BGR2RGB),
    )
...
def run_on_exp(exp_name, gpu=0):
    ...
    with open(results_file, "a") as f:
        string = tabulate(
            data,
            headers="firstrow",
            floatfmt=(".1f", ".1f", ".1f", ".1f", ".5f", ".3f", ".1f", ".5f"),
        )
        print(string)
        print(string, file=f)
