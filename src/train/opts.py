...
class Opts:
    ...
    def __init__(self, validate=False):
        ...
        p.register(
            "type", None, identity
        )  # Fix to make ArgumentParser serializable for threading

        # General args
        p.add_argument("--gpu", type=int, default=0, help="CUDA GPU to use (-1 = CPU)")
        p.add_argument(
            "--silent", action="store_true", help="Do not print progress bars to stdout"
        )

        # Dataset args
        p.add_argument("--left_only", action="store_true", help="Only run on left feet")
        p.add_argument(
            "--tpose_only", action="store_true", help="Only run on Tpose feet"
        )
        p.add_argument(
            "--full_caching",
            action="store_true",
            help="Fully cache dataset for faster loading.",
        )
        p.add_argument(
            "--train_and_val",
            action="store_true",
            help="Train and val in one dataset (for PCA).",
        )

        p.add_argument(
            "--low_poly_meshes",
            action="store_true",
            help="Load low polygon (10-20K faces) meshes",
        )
        p.add_argument(
            "--low_res_textures",
            action="store_true",
            help="Load low resolution (1K) textures",
        )
        ...
        p.add_argument(
            ...
            default=None,
            type=int,
            help="Number of training feet. Defaults to None (all feet)",
        )
        p.add_argument(
            "--n_val",
            default=None,
            type=int,
            help="Number of val feet. Defaults to None (all feet)",
        )
        ...
        p.add_argument(
            "--shuffle", action="store_true", help="Shuffle train & val dataloaders"
        )

        # Model arguments
        p.add_argument(
            "--model_type",
            choices=["neural", "pca", "vertexfeatures", "supr"],
            default="neural",
        )
        p.add_argument(
            "--load_model",
            default="",
            help="Load previous model file (overrides other model args)",
        )
        ...
        p.add_argument(
            "--reg",
            action="store_true",
            help="Run registration stage before training network.",
        )
        ...
        # NET
        p.add_argument(
            "--no_net_train", action="store_true", help="Don't train network"
        )
        ...
        p.add_argument(
            "--val_crit", default="chamf", type=str, help="Validation critieria used."
        )

        # LATENT REFINEMENT
        p.add_argument(
            "--no_latent_refinement", action="store_true", help="Don't refine latents"
        )
        p.add_argument("--latent_epochs", type=int, default=500)
        p.add_argument(
            "--latent_optim", type=str, choices=["SGD", "Adam"], default="Adam"
        )
        ...
        default_weights = dict(
            ...
            pix=1.0,
            vgg_perc=0.1,
            restyle_perc_lat=0.25,
            restyle_perc_feat=1.0,
            tex=1.0,
            sil=5.0,
            restyle_perc_cluster=1.0,
            cont_pose=1.0,
        )
        ...
        p.add_argument(
            ...
            action="store_true",
            help="Predict restyle features per vertex",
        )
        p.add_argument(
        ...
        # Renderer args
        p.add_argument(
            "--num_views", type=int, default=5, help="Views to render per foot"
        )
        p.add_argument(
            "--special_view_type",
            default=None,
            choices=[None, "topdown", "sample_arc"],
        ...
        p.add_argument(
            "--copy_over_masking",
            action="store_true",
            help="Take masking from GT, apply to predicted",
        )
        p.add_argument(
            "--mask_out_pred",
            action="store_true",
        ...
        p.add_argument(
            "--save_dir",
            default="models",
            help="Directory in which --model_name directory will be created",
        )

        p.add_argument(
            "--save_meshes_on_end_only",
            action="store_true",
        ...
        p.add_argument(
            "--net_repeat_dataset",
            default=1,
            help="Repeat train dataset N times within each epoch",
        )
        p.add_argument(
            "--gt_z_cutoff", default=None, help="Enforce z cutoff height of GT mesh"
        )
        ...
        p.add_argument(
            "--vf_optim_features",
            action="store_true",
            help="VF model, allow vertex features to optimise",
        )
        p.add_argument(
            "--vf_optim_deforms",
            action="store_true",
            help="VF model, allow vertex deforms to optimise",
        )
        p.add_argument(
            "--vf_optim_reg",
            action="store_true",
            help="VF model, allow reg to optimise",
        )
        ...
        p.add_argument(
            "--use_pose_code",
            action="store_true",
            help="Parameterise pose as well [experimental]",
        )
        ...
        p.add_argument(
            "--no_loading",
            action="store_true",
            help="For evaluation - do not load from previous optims",
        )
        p.add_argument(
            "--unseen", action="store_true", help="Evaluate on 'unseen' images"
        )
    ...
    def validate_training(self):
        ...
        if not self.no_net_train:
            assert any(
                losses.values()
            ), "At least one loss must be used for net training!"
            assert losses[
                self.val_crit
            ], f"Val crit set to {self.val_crit}, but this loss isn't used"
        ...
        assert not (
            self.restrict_3d_n_train and (self.restrict_3d_train_key is not None)
        ), "Both restrict_3d_train and restrict_3d_train_key cannot be used simulatenously"

    def net_train_kwargs(self):
        """Return a dictionary of True/False for the losses to be fed into the model's forward loop."""
        return dict(
            chamf=self.chamf_loss,
            smooth=self.smooth_loss,
            texture=self.texture_loss,
            pix=self.pix_loss,
            sil=self.sil_loss,
