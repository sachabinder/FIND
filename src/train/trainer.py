...
class Trainer:
    def __init__(
        ...
        opts: Opts,
        latent_vectors_train: list = None,
        latent_vectors_val: list = None,
        val_optim=None,
        device="cuda",
        n_repeat=1,
    ):
        ...
        self.best_epochs = (
            []
        )  # Store each time a new best epoch is found for plotting graphs
    ...
    def train_epoch(self, epoch, save_model=False, model_kwargs={}):
        ...
        with tqdm(
            self.train_loader, bar_format="{l_bar}{bar:10}{r_bar}{bar:-10b}"
        ) as tqdm_it:
            end = perf_counter()
            ...
            for n in range(self.n_repeat):
                for batch in tqdm_it:
                    ...
                    batch.update(
                        **self.sample_latent_vectors(
                            batch, latent_vectors=self.latent_vectors_train
                        )
                    )
                    ...
                    loss, loss_dict = self.model(
                        batch, epoch, opts=self.opts, **model_kwargs
                    )
                    ...
                    desc = f"[{epoch}] " + "|".join(
                        [f"{k}:{np.mean(v):.2f}" for k, v in epoch_losses.items()]
                    )
    ...
    def val_epoch(self, epoch, model_kwargs={}):
        ...
        with tqdm(
            self.val_loader, bar_format="{l_bar}{bar:10}{r_bar}{bar:-10b}"
        ) as tqdm_it:
            end = perf_counter()
            for batch in tqdm_it:
                # Add latent vectors to batch
                batch.update(
                    **self.sample_latent_vectors(
                        batch, latent_vectors=self.latent_vectors_val
                    )
                )
                batch = batch_to_device(batch, self.device)
                ...
                tqdm_it.set_description(
                    ...
                    + "|".join(
                        [
                            f"{pretty_print_loss(k)}:{np.mean(v):.2f}"
            ...
            self.log[epoch]["val_loss"] = {
                pretty_print_loss(k): v for k, v in epoch_losses.items()
            }
    ...
    def plot(self, out_loc):
        ...
        all_keys = ["Loss"] + sorted(
            [k for k in all_keys if k != "Loss"]
        )  # Make sure overall loss is first
        ...
        for n, k in enumerate(all_keys):
            ...
            for epoch in self.log:
                ...
                if k in epoch_dict.get("train_loss", {}):
                    ...
                    train_y.append(epoch_dict["train_loss"][k])

                if k in epoch_dict.get("val_loss", {}):
                    val_x.append(epoch)
                    val_y.append(epoch_dict["val_loss"][k])
    ...
    def export_meshes(self, export_loc, is_train=False, export_gt=False):
        ...
        with tqdm(loader, bar_format="{l_bar}{bar:10}{r_bar}{bar:-10b}") as tqdm_it:
            ...
            for batch in tqdm_it:
                # Add latent vectors to batch
                latent_vectors = [self.latent_vectors_val, self.latent_vectors_train][
                    is_train
                ]
                batch.update(
                    **self.sample_latent_vectors(batch, latent_vectors=latent_vectors)
                )
                ...
                for i in range(len(res["meshes"])):
                    ...
                    with open(
                        os.path.join(export_loc, f"{batch['name'][i]}.obj"), "w"
                    ) as outfile:
                        outfile.write(obj_data)

                    if export_gt:
                        vertsgt, facesgt = i["verts"], i["faces"]
                        meshgt = trimesh.Trimesh(
                            vertices=vertsgt.cpu().detach().numpy(),
                            faces=facesgt.cpu().detach().numpy(),
                        )
                        ...
                        with open(
                            os.path.join(export_gt_loc, f"{batch['name'][i]:04d}.obj"),
                            "w",
                        ) as outfile:
                            outfile.write(obj_data)
