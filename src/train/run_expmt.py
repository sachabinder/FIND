...
def main(cfg_file):
    ...
    for expmt_name in cfg["EXPERIMENTS"]:
        ...
        if load_from_reg:
            if load_from_reg_loc and not opt_dict.get(
                "reg", False
            ):  # If this experiment doesn't have a reg stage, and there is a reg to load from
                ...
                assert opt_dict[
                    "reg"
                ], "load_from_reg setting used, but this requires first experiment to have --reg flag on!"
    ...
    if not cfg["USE_THREADING"]:
        ...
        if (
            load_from_reg
        ):  # If registration is from first experiment, need to run this one first before queues
