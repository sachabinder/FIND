...
def train_network(
    ...
    :param model: Model to train/save
    :param trainer: Trainer for models
    :param logger: Logger for console output and logs
    :param train_dataset: TODO
    :param val_dataset: TODO
    :param out_dir: Output path of model saving
    :param checkpoint_dir: Output path for checkpoint saving
    :param num_epochs: Number of training epochs
    :param save_every: Interval of model saving
    :param val_every: TODO
    :param plot_name: Plot file name
    :param val_only: Only evaluate on validation
    """
    val_best = 1e3
    best_epoch = 0
    ...
    for epoch in range(num_epochs):
        ...
        save_epoch = epoch % save_every == 0
        # check if should save the model at this epoch
        save_model = save_epoch or is_last
        ...
        model_kwargs["render_foot"] = (
            any(k in args.render_losses and v for k, v in model_kwargs.items())
            or save_renders
        )
        ...
        if val_only or save_model:
            ...
            if metric <= val_best:
                ...
                if is_last:
                    trainer.export_meshes(
                        export_loc=os.path.join(args.vis_dir, "train_meshes"),
                        is_train=True,
                        export_gt=False,
                    )
                    trainer.export_meshes(
                        export_loc=os.path.join(args.vis_dir, "val_meshes"),
                        is_train=False,
                        export_gt=False,
                    )
                ...
                print(
                    f"NOT NEW BEST. epoch = {epoch}, metric = {metric:.4f}, best = {val_best:.4f}"
                )
...
def run(args: Opts):
    ...
    if args.gpu > -1:
        torch.cuda.set_device(
            args.gpu
        )  # Needed to avoid memory access errors for gpu > 0
    assert (
        device == "cpu" or torch.cuda.is_available()
    ...
    model = ModelWithLoss(
        ...
        use_shapevec=True,
        use_texvec=True,
        use_posevec=args.use_pose_code,
    ...
    trainer_reg = Trainer(
        ...
        val_optim=optim_reg,
        device=device,
    )  # Just learn template in stage 1
    trainer_net = Trainer(
        [optim_network],
        model,
        train_loader,
    ...
    if not args.no_net_train:
        ...
        train_network(
            ...
            num_epochs=args.net_epochs,
            save_every=args.net_save_every,
            val_every=args.net_val_every,
        )
    ...
    if not args.no_latent_refinement:
        ...
        train_network(
            ...
            out_dir=out_dir,
            checkpoint_dir=checkpoint_dir,
            num_epochs=args.latent_epochs,
            save_every=args.latent_save_every,
            val_every=args.latent_val_every,
            plot_name="latent",
            val_only=True,
        )
