...
class FeatureShader(nn.Module):
    ...
    def sample_textures(self, meshes: Meshes, fragments, texture=None):
        ...
        if texture is not None:
            ...
            return texture.sample_textures(
                fragments, faces_packed=meshes.faces_packed()
            )
    ...
    def forward(
        self, fragments, meshes: Meshes, feature_tex: TexturesVertex = None, **kwargs
    ) -> torch.Tensor:
        """
...
class FootRenderer(nn.Module):
    def __init__(
        ...
        self.cameras = FoVPerspectiveCameras(
            device=device, R=R, T=T, znear=0.1
        )  # Placeholder camera
        ...
        self.sil_raster_settings = RasterizationSettings(
            image_size=(image_size, image_size),
            max_faces_per_bin=max_faces_per_bin,
            blur_radius=np.log(1.0 / 1e-4 - 1.0) * sil_blend_params.sigma,
            faces_per_pixel=100,
            bin_size=bin_size,
        )
        ...
        self.sil_rasterizer = MeshRasterizer(
            cameras=self.cameras, raster_settings=self.sil_raster_settings
        )
        self.img_rasterizer = MeshRasterizer(
            cameras=self.cameras, raster_settings=self.img_raster_settings
        )

        # Shaders
        self.img_shader = SoftPhongShader(
            device=device,
            cameras=self.cameras,
            lights=self.lights,
            blend_params=img_blend_params,
        )
        self.sil_shader = SoftSilhouetteShader()
        self.feature_shader = FeatureShader(device=device)

        # Point cloud rendering
        pcl_raster_settings = PointsRasterizationSettings(
            image_size=image_size, radius=0.03, points_per_pixel=10
        )
        self.points_rasterizer = PointsRasterizer(
            cameras=self.cameras, raster_settings=pcl_raster_settings
        )
        self.points_renderer = PointsRenderer(
            rasterizer=self.points_rasterizer, compositor=AlphaCompositor()
        )
    ...
    def sample_views(
        ...
        R, T = look_at_view_transform(
            dist=distances, elev=elev, azim=azim, up=((1, 0, 0),)
    ...
    def rasterize(self, meshes_world, rasterizers: dict, cameras, **kwargs) -> dict:
        ...
        meshes_proj = rasterizers.get("img", rasterizers.get("sil")).transform(
            meshes_world, cameras=cameras, **kwargs
        )
        fragments = {}
    ...
    def forward(
        ...
        return_mask=False,
        mask_with_grad=True,
        mask_out_faces=False,
        masked_faces=None,
        keypoints=None,
        keypoints_blend=False,
        lights=None,
        return_mask_out_masks=False,
        ...
        cameras = FoVPerspectiveCameras(
            device=input_meshes.device, R=R, T=T, znear=0.02
        )
        ...
        rasterizers = {}
        if return_images or return_features:
            rasterizers["img"] = self.img_rasterizer
        if return_mask:
            rasterizers["sil"] = self.sil_rasterizer
        fragments = self.rasterize(meshes, rasterizers, cameras, lights=lights)

        if return_images:
            renders = self.img_shader(
                fragments["img"], meshes, cameras=cameras, lights=lights
            )
        ...
        if return_features:
            ...
            feat_blend_params = BlendParams(background_color=[0.0] * nfeat)
            feat_renders = self.feature_shader(
                fragments["sil"],
                meshes,
                feature_tex=feat_tex,
                blend_params=feat_blend_params,
            )
            feat_renders = feat_renders.reshape(
                num_meshes, num_views, self.image_size, self.image_size, -1
            )
        ...
        if return_mask:
            if mask_with_grad or not return_images:
                mask = self.sil_shader(fragments["sil"], meshes, cameras=cameras)[
                    ..., 3
                ].reshape(
                    out_shape_single
                )  # Get seg from alpha channel
        ...
        if mask_out_faces:  # Mask out faces of images and masks
            ...
            pix_to_face = pix_to_face.reshape(
                num_meshes, num_views, self.image_size, self.image_size
            ...
            for n in range(len(input_meshes)):
                if masked_faces is not None:
                    mask_faces = (
                        masked_faces[n]
                        if isinstance(masked_faces, list)
                        else masked_faces
                    )
                    ...
                    ), f"Mask out faces functionality requires masked_faces or TexturesUV - received `{type(meshes.textures)}`"
                ...
                if return_features:
                    mpix = mask_pix.unsqueeze(-1).expand(
                        -1, -1, -1, feat_renders.shape[-1]
                    )
                    feat_renders[n, mpix] = 0.0

        if keypoints is not None:  # Render keypoints onto scene
            n, K, _ = keypoints.shape
            keypoints.reshape(num_meshes, 1, -1, 3).expand(
                -1, num_views, -1, -1
            ).reshape(
                -1, K, 3
            )  # Stack in views dimension
            features = torch.zeros_like(keypoints)
            features[..., 0] = 1
            pcl = Pointclouds(points=keypoints, features=features)
            pcl_renders = self.points_renderer(pcl, cameras=cameras).reshape(
                out_shape_rgb
            )
            ...
            if keypoints_blend:
                ...
                out["keypoints_blend"] = (
                    ~pcl_mask * renders.clone() + pcl_mask * pcl_renders
                )
        ...
        if return_mask:
            out["mask"] = mask
        if return_mask_out_masks:
            out["mask_out_masks"] = torch.cat(mask_out_faces_masks).reshape(
                out_shape_single
            )
        if return_features:
