...
class TextureLossGTSpace(nn.Module):
    ...
    def forward(
        ...
        sampled_verts, sampled_gt_colours = sample_points_from_meshes(
            mesh_gt, num_samples=num_samples, return_textures=True
        )
...
class RestylePerceptualLoss(nn.Module):
    def __init__(self, pth="", classifier_pth=None, device="cuda"):
        ...
        self.encoder = RestyleEncoder(
            pth, device=device, classifier_src=classifier_pth
        ).eval()
    ...
    def crit(self, pred, gt, pred_mask=None, gt_mask=None):
        ...
        if pred_mask is not None:
            pred = pred * pred_mask.squeeze(0).unsqueeze(1)
        if gt_mask is not None:
            gt = gt * gt_mask.squeeze(0).unsqueeze(1)
    ...
    def debug_vis(
        ...
        """Visualize"""
        if pred_mask is not None:
            pred_feat = pred_feat * pred_mask.squeeze(0).unsqueeze(1)
        if gt_mask is not None:
            gt_feat = gt_feat * gt_mask.squeeze(0).unsqueeze(1)
        ...
        def fmap_to_im(fmap):
            ...
            im[:] = (((fmap - fmap.min()) / (fmap.max() - fmap.min())) * 255).astype(
                np.uint8
            )[:, :, None]
        ...
        for n, pred_rdr in enumerate(pred_rdrs):
            gt_im = np.hstack(
                [
                    rdr_to_im(gt_rdrs[n]),
                    *[
                        fmap_to_im(g.cpu().detach().numpy())
                        for g in gt_feat[n, vis_idxs]
                    ],
                ]
            )
            pred_im = np.hstack(
    ...
    def debug_cluster(
        self, pred_rdrs, gt_rdrs, pred_labels, gt_labels, out_dir="restyle_debug"
    ):
        """Visualize"""
        rdr_to_im = lambda arr: (255 * arr.cpu().detach().numpy()).astype(np.uint8)
        ...
        for n, pred_rdr in enumerate(pred_rdrs):
            gt_im = np.hstack(
                [
                    rdr_to_im(gt_rdrs[n]),
                    visualize_classes_argmax(gt_labels[n].cpu().detach().numpy()),
                ]
            )
            pred_im = np.hstack(
                [
                    rdr_to_im(pred_rdr),
                    visualize_classes_argmax(pred_labels[n].cpu().detach().numpy()),
                ]
            )
            out_im = np.vstack([gt_im, pred_im]).astype(np.uint8)
    ...
    def forward(
        ...
        assert mode in [
            ...
            "latent",
            "cluster",
        ], f"Mode `{mode}` not understood for Restyle loss."
        ...
        *_, H, W, _ = pred.shape
        ...
        _upsample = lambda feat: nn.functional.interpolate(
            feat, size=(H, W), mode="bilinear"
        )
        ...
        if mode == "feat":
            if pred_feat is None:
                pred_feat = self.encoder(
                    pred, return_features=True, target_feature_maps=feature_maps
                )["features"]
            else:
                pred_feat = {8: pred_feat}
            gt_feat = self.encoder(
                gt, return_features=True, target_feature_maps=feature_maps
            )["features"]
            ...
            if feature_maps is None:
                ...
                N = len(pred_feat)
                return sum(
                    self.crit(p, g, pred_masks, gt_masks) / N
                    for p, g in zip(pred_feat, gt_feat)
                )

            if feature_maps is not None:
                pred_feat_upsampled = torch.cat(
                    [_upsample(pred_feat[f]) for f in feature_maps], dim=1
                )
                gt_feat_upsampled = torch.cat(
                    [_upsample(gt_feat[f]) for f in feature_maps], dim=1
                )
                ...
                return self.crit(
                    pred_feat_upsampled, gt_feat_upsampled, pred_masks, gt_masks
                )
        ...
        if mode == "cluster":
            if pred_logit is None:
                pred_logit = self.encoder(
                    pred, return_features=True, target_feature_maps=feature_maps
                )["class_logits"]
            else:
                pred_logit = pred_logit

            gt_logit = self.encoder(
                gt, return_features=True, target_feature_maps=feature_maps
            )["class_logits"]

            if feature_maps != [8]:
                raise NotImplementedError(
                    "Classifier only works with exactly feat map 8 currently."
                )

            pred_logit_upsampled = _upsample(
                pred_logit
            )  # torch.cat([_upsample(pred_feat[f]) for f in feature_maps], dim=1)
            gt_logit_upsampled = _upsample(
                gt_logit
            )  # torch.cat([_upsample(gt_feat[f]) for f in feature_maps], dim=1)
            ...
            if debug:
                ...
                rdr_to_im = lambda arr: (255 * arr.cpu().detach().numpy()).astype(
                    np.uint8
                )
                ...
                def fmap_to_im(fmap):
                    im = np.zeros((*fmap.shape, 3))
                    im[:] = (
                        ((fmap - fmap.min()) / (fmap.max() - fmap.min())) * 255
                    ).astype(np.uint8)[:, :, None]
                    return im

                hmps = [
                    fmap_to_im(pred_logit_upsampled[0, n].cpu().detach().numpy())
                    for n in range(5)
                ]
                x = fmap_to_im(pred_labels[0].cpu().detach().numpy())
                im = np.hstack([rdr_to_im(pred[0].permute(1, 2, 0)), x, *hmps]).astype(
                    np.uint8
                )
                ...
                cv2.imwrite(
                    os.path.join(debug_dir, "_CELoss.png"),
                    fmap_to_im(full_loss[0].cpu().detach().numpy()),
                )
