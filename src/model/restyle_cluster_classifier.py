...
class RestyleFeatures(Dataset):
    def __init__(
        ...
        self.image_files = [
            os.path.join(
                self.image_dir,
                os.path.splitext(os.path.basename(feature_file))[0] + ".png",
            )
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        mask = (
            self.upsample(mask.reshape((1, 1, mask.shape[0], mask.shape[1]))).squeeze()
            > 0
        )
        feats = []
        for style in self.styles:
            sampled_feat = (
                self.upsample(torch.from_numpy(features[style]).unsqueeze(0))
                .squeeze()
                .numpy()
            )
            sampled_feat = sampled_feat.transpose((1, 2, 0)).reshape(
                (sampled_feat.shape[1] * sampled_feat.shape[2], -1)
            )
...
class FeatureClassifier(nn.Module):
    def __init__(self, num_layers=4, input_channels=100, num_classes=21):
        ...
        self.input = nn.Sequential(
            nn.Dropout(0.1), nn.Conv2d(input_channels, 512, 1), nn.LeakyReLU()
        )
        ...
        self.logit_layer = nn.Sequential(
            nn.Dropout(0.1), nn.Conv2d(int(512 / ((n + 2) ** 2)), num_classes, 1)
        )
...
if __name__ == "__main__":
    ...
    out = {
        "state_dict": d,
        "params": dict(num_layers=4, input_channels=256, num_classes=21),
    }
    torch.save(
        out, os.path.join("misc/restyle_classifier_models/feat_8_articulated.pth")
    )
