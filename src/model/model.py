...
def load_obj(loc, skip_materials=True):
    ...
    with open(loc, "rb") as f:
        mesh = trimesh.Trimesh(
            **trimesh.exchange.obj.load_obj(
                f, skip_materials=skip_materials, resolver=resolver
            )
        )
    return mesh
...
class LatentVector(nn.Module):
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(idx, str):
            ...
            ), f"Tried to access item {idx} from LatentVector {self.name}, LV does not have labels"
        ...
        if isinstance(idx, list):
            ...
            raise NotImplementedError(
                f"Didn't understand indexing of LatentVector {self.name}, type {type(idx)}"
            )
...
class Model(nn.Module):
    ...
    def load(cls, file, device="cuda", opts=None, **kwargs):
        ...
        assert (
            ext == ".pth"
        ), f"Generic models can only load from .pth files - received `{ext}` file."
        ...
        if opts.dont_load_latents:
            ...
            data["params"]["latent_labels"] = kwargs.get(
                "latent_labels", None
...
class NeuralDisplacementField(Model):
    def __init__(
        ...
        use_posevec=False,
        posevec_size=256,
        template_mesh_loc=None,
        device="cuda",
        restyle_features_per_vertex=False,
        restyle_cluster_per_vertex=False,
        use_avg_colour=False,
        opts=None,
        ...
        self.params = dict(
            ...
            val_size=val_size,
            use_shapevec=use_shapevec,
            shapevec_size=shapevec_size,
            use_texvec=use_texvec,
            texvec_size=texvec_size,
            use_posevec=use_posevec,
            posevec_size=posevec_size,
            restyle_features_per_vertex=restyle_features_per_vertex,
            restyle_cluster_per_vertex=restyle_cluster_per_vertex,
            use_avg_colour=use_avg_colour,
            latent_labels=latent_labels,
        )

        self.pe = ProgressiveEncoding(
            mapping_size=width, T=niter, d=input_dim, device=device
        )
        self.clamp = clamp
        ...
        if template_mesh_loc is not None:
            ...
            mesh = Meshes(
                verts.unsqueeze(0).to(device),
                faces.unsqueeze(0).to(device),
                textures=TexturesUV(
                    props.texture_images["material_0"].unsqueeze(0).to(device),
            ...
            _, samples = sample_points_from_meshes(
                mesh, num_samples=1000, return_textures=True
            )
        ...
        self.template_verts = nn.Parameter(
            verts.unsqueeze(0).float().to(device), requires_grad=False
        )
        self.template_faces = nn.Parameter(
            faces.unsqueeze(0).to(device), requires_grad=False
        )
        self.avg_col = nn.Parameter(avg_col.to(device), requires_grad=False)

        self.template_mesh = Meshes(
            verts=self.template_verts, faces=self.template_faces
        ).to(device)
        ...
        if use_shapevec:
            ...
            labels_val = (
                None if latent_labels is None else latent_labels.get("shape_val", None)
            )
            self.shapevec = LatentVector(
                train_size,
                vec_size=shapevec_size,
                name="shapevec_train",
                key="shape",
                labels=labels,
                device=device,
            )
            self.shapevec_val = LatentVector(
                val_size,
                vec_size=shapevec_size,
                name="shapevec_val",
                key="shape",
        ...
        if use_posevec:
            ...
            labels_val = (
                None if latent_labels is None else latent_labels.get("pose_val", None)
            )
            self.posevec = LatentVector(
                train_size,
                vec_size=shapevec_size,
                name="posevec_train",
                key="pose",
        ...
        if use_texvec:
            ...
            labels_val = (
                None if latent_labels is None else latent_labels.get("tex_val", None)
            )
            self.texvec = LatentVector(
                train_size,
                vec_size=texvec_size,
                name="texvec_train",
                key="tex",
                labels=labels,
                device=device,
            )
            self.texvec_val = LatentVector(
                val_size,
                vec_size=texvec_size,
                name="texvec_val",
                key="tex",
        ...
        labels_val = (
            None if latent_labels is None else latent_labels.get("reg_val", None)
        )
        self.reg = LatentVector(
            train_size,
            vec_size=9,
            name="reg_train",
            device=device,
        ...
        disp_input_size = (
            width
            + self.shapevec_size * self.use_texvec
            + self.posevec_size * self.use_posevec
        )
    ...
    def forward(self, pos, shapevec=None, texvec=None, posevec=None):
        ...
        if not self.onnx_mode:
            ...
            if (
                batch == 1 and shapevec is not None
            ):  # In case of no batch for pos, but batch for vectors
                batch = shapevec.shape[0]
                pos = pos.expand(batch, -1, -1)

            if shapevec is not None:
                shapevec = shapevec.unsqueeze(1).expand(
                    -1, np, -1
                )  # Add points dimension to shape

            if posevec is not None:
                posevec = posevec.unsqueeze(1).expand(
                    -1, np, -1
                )  # Add points dimension to pose

            if texvec is not None:
                texvec = texvec.unsqueeze(1).expand(
                    -1, np, -1
                )  # Add points dimension to texture
    ...
    def get_meshes(
        ...
        if reg is not None:
            ...
            T = (
                Transform3d(device=S.device)
                .scale(S)
                .rotate(R)
                .translate(centroid_offset)
            )
    ...
    def configure_template(self, state_dict, device="cuda"):
        ...
        self.template_verts = nn.Parameter(
            verts.float().to(device), requires_grad=False
        )
...
class PCAModel(Model):
    ...
    def get_meshes(self, shapevec=None, reg=None, **kwargs):
        ...
        offsets = (
            self.pca_coefs.unsqueeze(0) * shapevec.unsqueeze(1).unsqueeze(-1)
        ).sum(dim=2)
        ...
        col = (
            torch.full(X.shape, 0.5).float().to(meshes.device)
        )  # set all texture colour to grey
    ...
    def get_meshes_from_batch(self, batch, is_train=True, **kwargs):
        ...
        res = self.get_meshes(shapevec=batch.get(svec, None), reg=batch.get(reg, None))
    ...
    def load_from_mat(cls, src, device, **kwargs):
        ...
        state_dict = {}
        state_dict["template_verts"] = (
            torch.from_numpy(data["pcaMean"].reshape(-1, 3)).unsqueeze(0).float()
        )
        state_dict["template_faces"] = (
            torch.from_numpy(data["mesh"]).unsqueeze(0).long() - 1
        )

        V = state_dict["template_verts"].shape[1]
        state_dict["pca_coefs"] = (
            torch.from_numpy(data["pcaCoefs"])
            .reshape(V, 3, -1)
            .permute(0, 2, 1)
            .float()
        )  # V x B x 3
    ...
    def load(cls, file, device="cuda", opts=None, **kwargs):
        if file.endswith(".pth"):
            ...
            raise NotImplementedError(
                f"Filetype `{os.path.splitext(file)[-1]}` for PCA model not understood"
            )
    ...
    def configure_template(self, state_dict, device="cuda"):
        ...
        self.pca_var = nn.Parameter(
            state_dict["pca_var"].to(device), requires_grad=False
        )  # [B]
        self.pca_coefs = nn.Parameter(
            state_dict["pca_coefs"].to(device), requires_grad=False
        )  # [V x B x 3]

        V, B, _ = self.pca_coefs.shape

        self.shapevec = LatentVector(
            self.train_size, vec_size=B, name="shapevec_train", device=device
        )
        self.shapevec_val = LatentVector(
            self.val_size, vec_size=B, name="shapevec_val", device=device
        )
        self.latent_vectors_train += [self.shapevec]
        self.latent_vectors_val += [self.shapevec_val]
...
class SUPRModel(Model):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.pose_train = LatentVector(
            train_size,
            vec_size=num_joints * 3,
            name="pose_train",
            device=device,
            init_values=pose_init,
        )
        self.pose_val = LatentVector(
            val_size,
            vec_size=num_joints * 3,
            name="pose_val",
            device=device,
        ...
        self.betas_train = LatentVector(
            train_size, vec_size=num_betas, name="betas_train", device=device
        )
        self.betas_val = LatentVector(
            val_size, vec_size=num_betas, name="betas_val", device=device
        )

        trans_init = (
            -supr_model.v_template.mean(dim=0).cpu().numpy()
        )  # Initialize trans to centre object
        self.trans_train = LatentVector(
            train_size,
            vec_size=3,
            name="trans_train",
            init_values=trans_init,
            device=device,
        )
        self.trans_val = LatentVector(
            val_size,
            vec_size=3,
            name="trans_val",
            init_values=trans_init,
        ...
        faces = [
            [loop_verts[0], b, c] for b, c in zip(loop_verts[1:], loop_verts[2:])
        ]  # the necessary faces
        self.supr_model.faces = torch.cat(
            [self.supr_model.faces, torch.tensor(faces)], dim=0
        )
    ...
    def configure_params(self):
        ...
        self.latent_params = make_params_list(
            self.betas_train, self.betas_val, self.pose_train, self.pose_val
        )
    ...
    def get_meshes(self, poses, betas, trans, reg, **kwargs):
        ...
        meshes = Meshes(
            ...
            faces=self.supr_model.faces.unsqueeze(0).expand(batch_size, -1, -1),
        )

        col = (
            torch.full(deformed_verts.shape, 0.5).float().to(meshes.device)
        )  # set all texture colour to grey
        meshes.textures = TexturesVertex(col)
    ...
    def get_meshes_from_batch(self, batch, is_train=True, **kwargs):
        poses, betas, trans, reg = [
            f"{a}_{['val', 'train'][is_train]}"
            for a in ["pose", "betas", "trans", "reg"]
        ]
        res = self.get_meshes(
            poses=batch[poses], betas=batch[betas], trans=batch[trans], reg=batch[reg]
        )
    ...
    def load_from_npy(cls, src, device, opts=None, **kwargs):
        """Load data from .mat file"""
        return cls(
            SUPR(src, num_betas=kwargs.get("num_betas", 10), device=device),
            **kwargs,
            device=device,
        )
    ...
    def load(cls, file, device="cuda", opts=None, **kwargs):
        if file.endswith(".pth"):
            ...
            raise NotImplementedError(
                f"Filetype `{os.path.splitext(file)[-1]}` for SUPR model not understood"
            )
    ...
    def load_from_pth(cls, file, device="cuda", opts=None, **kwargs):
        ...
        supr_model = SUPR(
            data["params"]["path_model"],
            num_betas=data["params"]["num_betas"],
            device=device,
        )
        ...
        **kwargs,
        ...
        if template_mesh_loc is not None:
            ...
            if (template_tex.verts_uvs_padded()[0, -1] == 0).all():
                ...
                vt = (
                    template_tex.verts_uvs_padded().shape[1] - 1
                )  # index corresponding to this final vertex
                masked_faces = torch.argwhere(
                    torch.all(face_uv == vt, dim=-1)
                ).flatten()
        ...
        self.features = nn.Parameter(
            torch.zeros(V, feature_size).to(device), requires_grad=True
        )
        self.colours = nn.Parameter(
            torch.full((V, 3), 0.5).unsqueeze(0).to(device), requires_grad=True
        )
...
model_zoo = dict(
    neural=NeuralDisplacementField,
    pca=PCAModel,
    vertexfeatures=VertexFeaturesModel,
    supr=SUPRModel,
)
...
class ModelWithLoss(nn.Module):
    def __init__(self, *args, opts: Opts = None, device="cuda", **kwargs):
        ...
        if opts.use_restyle():
            self.restyle_perc_loss = RestylePerceptualLoss(
                pth=opts.restyle_encoder_path,
                device=device,
    ...
    def forward(
        ...
        vgg_perc=False,
        sil=False,
        restyle_perc_lat=False,
        restyle_perc_feat=False,
        restyle_perc_cluster=False,
        cont_pose=False,
        render_foot=False,
        save_renders=False,
        render_dir="_pix",
        ...
        use_z_cutoff=False,
        gt_z_cutoff=None,
        restyle_feature_maps=None,
        no_displacement=False,
        return_renders=False,
        copy_mask_out=True,
        mask_out_pred_faces=False,
    ):
        res = self.model.get_meshes_from_batch(
            batch, is_train=is_train, no_displacement=no_displacement
        )
        ...
        if apply_loss_3d:
            if chamf:
                loss_chamf = self.def_loss(
                    self.model,
                    res,
                    batch,
                    epoch,
                    z_cutoff=0.07 if use_z_cutoff else None,
            ...
            if texture:
                ...
                raw_losses["loss_tex"] = self.col_loss(
                    self.model,
                    batch,
                    shapevec=batch.get(svec, None),
                    texvec=batch.get(tvec, None),
        ...
        if cont_pose:
            ...
            if batch["pose_code"].shape[0] > 1:
                raw_losses["loss_cont_pose"] = self.contrastive_loss(
                    batch[pvec], batch["pose_code"]
                )
        ...
        if render_foot or save_renders:
            if opts.special_view_type:
                ...
                if opts.special_view_type == "topdown_5":
                    R, T = self.rdr.combine_views(
                        *self.rdr.view_from("topdown"),
                        *self.rdr.sample_views(
                            nviews=5,
                            dist_mean=0.3,
                            dist_std=0,
                            elev_min=-90,
                ...
                R, T = self.rdr.sample_views(
                    ...
                    azim_min=-90,
                    azim_max=90,
                )  # Get same viewpoints for GT and Pred
            ...
            if opts.restyle_features_per_vertex or opts.restyle_cluster_per_vertex:
                ...
                feat_dict["features"] = (
                    res["fpv"] if opts.restyle_features_per_vertex else res["cpv"]
                )

            # If masking out faces, add these to rendering dirctionary
            mask_out_dict = {}
            if mask_out_pred_faces:
                mask_out_dict.update(
                    dict(mask_out_faces=True, masked_faces=self.model.masked_faces)
                )
            ...
            if copy_mask_out:
                ...
                pred_rdrs["image"][
                    gt_rdrs["mask_out_masks"]
                    .unsqueeze(-1)
                    .expand_as(pred_rdrs["image"])
                ] = 1.0
                pred_rdrs["mask"][
                    gt_rdrs["mask_out_masks"].expand_as(pred_rdrs["mask"])
                ] = 0.0
            ...
            if sil:
                raw_losses["loss_sil"] = self.sil_loss(
                    pred_rdrs["mask"], gt_rdrs["mask"]
                )

            N, M, H, W, _ = gt_rdrs["image"].shape
            args = (
                pred_rdrs["image"].view(-1, H, W, 3),
                gt_rdrs["image"].view(-1, H, W, 3),
            )
            if vgg_perc:
                raw_losses["loss_vgg_perc"] = self.perc_loss(*args)
            if restyle_perc_lat:
                raw_losses["loss_restyle_perc_lat"] = self.restyle_perc_loss(
                    *args, mode="latent", H=H, W=W
                )
            if restyle_perc_feat:
                k = dict()
                if not opts.restyle_no_masking:
                    k.update(
                        dict(gt_masks=gt_rdrs["mask"], pred_masks=pred_rdrs["mask"])
                    )
                ...
                raw_losses["loss_restyle_perc_feat"] = self.restyle_perc_loss(
                    *args,
                    mode="feat",
            ...
            if restyle_perc_cluster:
                ...
                if opts.restyle_cluster_per_vertex:
                    k["pred_logit"] = (
                        pred_rdrs["features"].view(N * M, H, W, -1).permute(0, 3, 1, 2)
                    )

                if not opts.restyle_no_masking:
                    k.update(
                        dict(
                            gt_masks=gt_rdrs["mask"].view(N * M, H, W),
                            pred_masks=pred_rdrs["mask"].view(N * M, H, W),
                        )
                    )

                (
                    raw_losses["loss_restyle_perc_cluster"],
                    encodings,
                ) = self.restyle_perc_loss(
                    *args,
                    mode="cluster",
                    feature_maps=restyle_feature_maps,
                    debug=save_renders,
        ...
        if save_renders:
            ...
            gt_stacked = np.vstack(
                gt_rdrs["image"].reshape(-1, H, W, 3).cpu().detach().numpy()
            )
            pred_stacked = np.vstack(
                pred_rdrs["image"].reshape(-1, H, W, 3).cpu().detach().numpy()
            )
            out_stacked = np.hstack([gt_stacked, pred_stacked])
            cv2.imwrite(
                f"{render_dir}/{epoch:04d}_{idx:02d}.png",
                cv2.cvtColor((out_stacked * 255).astype(np.uint8), cv2.COLOR_BGR2RGB),
            )

        losses = {
            k: v * getattr(opts, k.replace("loss", "weight"))
