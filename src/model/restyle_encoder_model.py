...
nn = torch.nn
from src.model.restyle_encoder.utils.inference_utils import (
    run_on_batch,
    get_average_image,
)
...
class TruncatedGenerator(Generator):
    ...
    def forward(
        ...
        for conv1, conv2, noise1, noise2, to_rgb in zip(
            ...
            if terminate_after_features and (
                target_feature_maps is not None and i > max(target_feature_maps)
            ):
...
class TruncatedpSp(pSp):
    ...
    def __init__(self, opts):
        ...
        self.decoder = TruncatedGenerator(
            self.opts.output_size, 512, 2, channel_multiplier=1
        )
    ...
    def forward(
        ...
        if return_latents:
            out["latent"] = result_latent
        if return_features:
            out["features"] = result_features
...
class RestyleEncoder(nn.Module):
    def __init__(
        self,
        src="misc/restyle_encoder_models/rendered_gt_top_views_tpose_128_slice.pt",
        device="cuda",
        classifier_src=None,
    ):
        super().__init__()
        ...
        self.transform_np = Encode128Transforms(opts).get_transforms()[
            "transform_inference"
        ]
        ...
        if classifier_src is not None:
            self.classifier = FeatureClassifier.load(
                "misc/restyle_classifier_models/feat_8_articulated.pth"
            ).to(device)
    ...
    def __call__(
        ...
        if numiter > 1:
            raise NotImplementedError("Currently only have single iter Restyle")
        avg_image_for_batch = self.avg_image.unsqueeze(0).repeat(
            input_cuda.shape[0], 1, 1, 1
        )
        ...
        if self.classifier:
            if 8 not in target_feature_maps:
                raise NotImplementedError(
                    "Classifier currently only works with feature map 8"
                )
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        for i in range(10):
            np.save(
                "misc/classifier_debug/feat_maps",
                res["features"][8].cpu().detach().numpy(),
            )
